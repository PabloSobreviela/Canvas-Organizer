...
def extract_course_code(name: str) -> str:
    """
    Extracts a short code like 'CS 1331' or 'MATH 2551' from a course name.
    Falls back to initials if no pattern is found.
    """
    if not name:
        return "UNK"

    # Pattern: 2-4 uppercase letters, optional space, 3-4 digits (e.g., CS 1331, MATH2551)
    match = re.search(r'\b([A-Z]{2,4})\s?(\d{3,4})\b', name)
    if match:
        return f"{match.group(1)} {match.group(2)}"
                                            ...
                                            if link['is_file']:
                                                if not any(f.get("url") == link['url'] for f in files_to_download):
                                                    files_to_download.append({
                                                        ...
                                                        "file_id": link.get(
                                                            "file_id") or f"link_{abs(hash(link['url']))}"
                                                    })
...
def sync_assignments():
    ...
    for row in rows:
        result.append({
            ...
            "nam": row["name"],
            "des": row["description"],
            # ship ONE due date to client (prevents frontend from needing old date fields)
            "due": row["normalized_due_at"] if row["normalized_due_at"] is not None else row["original_due_at"],
            "st": row["status"],
            "cat": row["category"],
        })
...
def resolve_course_dates():
    ...
    print(
        f"[GEMINI] Resolving {len(assignments)} Canvas assignments using {len(files)} files and {len(announcements)} announcements")
    if discover_new:
    ...
    try:
        ...
        discovered = [r for r in gemini_results if not (r.get("cid") or r.get("canvas_assignment_id"))]

        # keep these assignments even if AI returns wrapper fields you don't use yet
    except Exception as e:
        print(f"❌ Gemini resolve failed: {e}")
        import traceback
        traceback.print_exc()
        return jsonify({"error": "AI resolve failed", "detail": str(e)}), 500
    ...
    gemini_assignment_ids = {
        (r.get("cid") or r.get("canvas_assignment_id"))
        for r in canvas_updates
        if (r.get("cid") or r.get("canvas_assignment_id"))
    }
    ...
    for r in canvas_updates:
        # Accept BOTH new compact keys and old keys (safe rollout)
        ...
        # Dates from AI are now clean YYYY-MM-DD strings in local time.
        # No timezone shifting needed.
        ...
        # ---------------------------------------------------------
    if discovered:
        print(f"   🧹 Clearing {discovered_count} old discovered items to prevent duplicates...")
        cur.execute("""
            DELETE FROM assignments_normalized 
            WHERE course_id = ? AND canvas_assignment_id IS NULL
        """, (course_id,))
        conn.commit()  # Commit immediately so the SELECTs below don't find them
    # ---------------------------------------------------------
    for r in discovered:
        # Check both keys: 'st' (minified) and 'status' (full)
        status_val = r.get("st") or r.get("status")

        if status_val != "DISCOVERED":
            continue
        ...
        # Model no longer emits provenance/confidence/reason
        confidence = None
        source = "Schedule!"
        ...
        if model_category not in ("ASSIGNMENT", "QUIZ", "LECTURE", "READING", "EXAM", "ATTENDANCE", "PLACEHOLDER"):
            model_category = None

        if force_assignment:
            category = "ASSIGNMENT"
            deliverable = 1
            # Inside the 'for r in discovered' loop:
        else:
            category, deliverable = infer_category_from_discovered_item(name, desc)
            if model_category:
                category = model_category
                # FIX: Add "LECTURE" to this list so deliverable becomes 0
                deliverable = 0 if category in ("READING", "ATTENDANCE", "PLACEHOLDER", "LECTURE") else 1

        # ✅ FIX 2: Fix the Logic Gate. 'not in' ensures we allow undated Lectures/Readings.
        if not due and category not in ("READING", "LECTURE"):
            continue

        # Upsert into assignments_normalized (canvas_assignment_id NULL => discovered)
        if due is None:
            date_query = "AND normalized_due_at IS NULL"
            params = (course_id, category)
        else:
            date_query = "AND normalized_due_at = ?"
            params = (course_id, category, due)

        # ... (Rest of your SQL insert/update logic remains the same)
...
def canvas_courses():
    ...
    # 1. Fetch from Canvas
    r = requests.get(f"{base_url}/api/v1/courses?per_page=100", headers=canvas_headers(token))
    if r.status_code != 200: return jsonify(r.json())
    courses = r.json()

    # 2. Fetch AI overrides from DB
    conn = get_db()
    # Check if table exists before querying (in case migration didn't run)
    try:
    ...
    for c in courses:
        cid = str(c.get("id"))
        if cid in metadata:
            c["course_code"] = metadata[cid]  # Use AI value
        else:
            # Prefer Canvas's own course_code (often already has MATH 2551 / CS 1331),
            # then try sis_course_id, then name.
            ...
            # If we can't find something like "MATH 2551", fall back to Canvas's course_code or name
            c["course_code"] = code or raw_code or name

    return jsonify(courses)

# Replace your existing @app.route("/api/reading_items/<course_id>"...)
@app.route("/api/reading_items/<course_id>", methods=["GET"])
def reading_items(course_id):
    conn = get_db()
    cur = conn.cursor()

    # OLD QUERY: Read from the obsolete table
    # cur.execute("SELECT name, details, due_at FROM reading_items ...")

    # NEW QUERY: Read from the unified AI table (assignments_normalized)
    # We explicitly fetch items categorized as LECTURE, READING, or those marked as "not a deliverable"
    cur.execute("""
        SELECT 
            name, 
            description as details, 
            normalized_due_at as due_at
        FROM assignments_normalized
        WHERE course_id = ? 
