...
def init_firebase():
    ...
    # (and so other env vars like GCP_PROJECT_ID can't accidentally break auth).
...
def get_course_announcements(user_id: str, course_id: str, canvas_credential_key: str = None) -> List[Dict]:
    """Get announcements for a course ordered by date (oldest first)."""
    db = get_db()
    announcements_ref = db.collection('users').document(user_id).collection('announcements')
    # Equality-only query avoids composite-index requirements. Ordering in memory keeps
    # behavior stable even when firestore.indexes.json order direction differs from code.
    ...
    announcements.sort(key=lambda row: (row.get('postedAt') is None, str(row.get('postedAt') or '')))
...
def save_ai_usage_log(
    ...
    """
    Persist a single AI usage event for later inspection.
    """
    db = get_db()
