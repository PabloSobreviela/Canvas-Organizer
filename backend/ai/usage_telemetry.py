...
def _read_usage_field(usage_metadata: Any, field_name: str) -> int:
    ...
    # OpenAI SDK response object path.
...
def _pricing_for_model(model_name: str) -> Dict[str, Any]:
    ...
    global_input_env = os.getenv("LLM_INPUT_COST_PER_1M_USD")
    global_output_env = os.getenv("LLM_OUTPUT_COST_PER_1M_USD")
...
def _setup_otel_logger() -> bool:
    ...
    try:
        ...
        print(f"[WARN] OpenTelemetry unavailable for AI usage logs: {e}")
        _OTEL_ENABLED = False
        return False
    ...
    try:
        ...
        _OTEL_LOGGER = otel_logs.get_logger("canvas-organizer.ai.llm", "1.0.0")
        _OTEL_ENABLED = True
        return True
    except Exception as e:
...
def emit_usage_log(payload: Dict[str, Any]) -> None:
    ...
    if _setup_otel_logger() and _OTEL_LOGGER is not None:
        try:
            ...
            print(f"[WARN] Failed to emit OpenTelemetry AI usage log: {e}")

    print(f"[AI_USAGE] {json.dumps(attributes, ensure_ascii=True, sort_keys=True)}")
