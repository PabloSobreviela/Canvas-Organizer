...
def normalize_discovered_name_for_dedupe(raw_name: str) -> str:
    ...
    for word, number in ordinal_map.items():
        text = re.sub(rf"\b{word}\b", f" {number} ", text)
...
if USE_FIRESTORE:
    ...
    def auth_canvas_login():
        return canvas_oauth_login()

    @app.route("/api/auth/canvas/callback", methods=["GET"])
    @limiter.limit("20/minute")
    def auth_canvas_callback():
        return canvas_oauth_callback()

    @app.route("/api/auth/logout", methods=["POST"])
    def auth_logout():
        return canvas_oauth_logout()
...
def sync_announcements():
    ...
    valid_course_ids = [str(cid).strip() for cid in course_ids if str(cid).strip()]
    ...
    for cid_str in valid_course_ids:
        try:
            r = requests.get(
                f"{base_url}/api/v1/announcements",
                headers=canvas_headers(token),
                params=[("context_codes[]", f"course_{cid_str}")],
                timeout=CANVAS_REQUEST_TIMEOUT_SECONDS,
            )
        except Exception as exc:
            fetch_errors.append({"course_id": cid_str, "status": f"request_error:{type(exc).__name__}"})
            continue

        if r.status_code != 200:
            fetch_errors.append({"course_id": cid_str, "status": r.status_code})
            continue
        try:
            batch = r.json()
        except ValueError:
            fetch_errors.append({"course_id": cid_str, "status": "invalid_json"})
            continue
        if isinstance(batch, list):
            announcements.extend(batch)
            successful_fetches += 1
        else:
            fetch_errors.append({"course_id": cid_str, "status": "unexpected_response"})

    # Return 200 even when no announcements exist (empty is normal).
    # Only return 400 when every request actually errored out.
    if successful_fetches == 0 and fetch_errors:
        return jsonify({
            "error": "Failed to fetch announcements from all courses",
    ...
    if USE_FIRESTORE:
        for a in announcements:
            ...
            try:
                ...
                save_errors += 1
                logger.warning("Failed to save announcement %s: %s", a.get("id"), exc)
    else:
        # Local mode: SQLite
        conn = get_db()
        cur = conn.cursor()
    ...
    out = {"inserted": saved_count, "fetched": len(announcements)}
    if fetch_errors:
        out["partial_failures"] = fetch_errors
    if save_errors:
...
def sync_assignments():
    ...
    try:
        ...
        if "401" in err_str:
            detail = "Canvas rejected the API token (401 Unauthorized). Reconnect Canvas with a valid token."
            status_code = 401
        elif "403" in err_str:
            ...
            detail = "Canvas API request timed out. The Canvas server may be slow or unreachable."
        elif "ConnectionError" in err_str or "connection" in err_str.lower():
            detail = "Could not connect to Canvas. The Canvas server may be down."
    ...
    if USE_FIRESTORE:
        ...
        for a in assignments:
            ...
            assignment_status = 'OK' if (due_at or normalized_due) else 'MISSING_DUE_DATE'
        ...
        for a in assignments:
            ...
            if row is None:
                ...
                status = "OK" if (due_at or normalized_due) else "MISSING_DUE_DATE"
