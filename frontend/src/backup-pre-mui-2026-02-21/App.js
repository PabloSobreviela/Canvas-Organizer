...
function buildInCourseDedupeKey(item) {
  const assignmentToken = extractAssignmentIdFromItem(item, item?.courseId);
...
function dedupeAssignmentsWithinCourse(items) {
  ...
  for (const item of Array.isArray(items) ? items : []) {
    const key = buildInCourseDedupeKey(item);
  ...
  useEffect(() => {
    ...
    const unsubscribe = onAuthChange(async ({ user, token }) => {
      ...
      if (user) {
        ...
        await loadCachedData({ authToken: token || undefined, userId: user.uid });
...
.theme-app .focus\\:border-blue-500:focus { border-color: var(--accent) !important; }
      ...
      `}</style>
        ...
        <div className="flex flex-1 overflow-hidden">
          {/* SIDEBAR */}
          {!sidebarCollapsed && (
            <aside className="w-[230px] border-r border-zinc-900 bg-black flex flex-col overflow-hidden">
              ...
              <div className="px-4 py-4 mt-auto border-t border-zinc-900">
                <button
                  onClick={() => {
                    setShowSyncProgressPopover(false);
